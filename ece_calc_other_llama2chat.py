        ...
        for dic in test_data:
            ...
            if demo_num == 0:
                if rationale is False:
                    if task == 'cb' or task == 'anli':
                        prompt = no_rationale_prompt.format(premise=dic['premise'], hypothesis=dic['hypothesis'])
                    else:
                        prompt = no_rationale_prompt.format(text=dic['text'])
                else:
                    if task == 'cb' or task == 'anli':
                        prompt = rationale_prompt.format(premise=dic['premise'], hypothesis=dic['hypothesis'])
                    else:
                        prompt = rationale_prompt.format(text=dic['text'])
            else:
                if task == 'cb' or task == 'anli':
