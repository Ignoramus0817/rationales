...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer, data_args) -> Dict:
    ...
    def _preprocess(examples):
        if 'gsm8k' not in data_args.task_name:
            if data_args.task_name != 'nli':
                sources = [
                    prompt_template.format(text=_t)
                    for _t in examples['text']
                ]
            else:
                sources = [
                    prompt_template.format(premise=_p, hypothesis=_h)
        ...
        source_chat = []
        for _s in sources:
            _source_conversation = [{'role': 'user', 'content': _s}]
            _s_chat_id = tokenizer.apply_chat_template(_source_conversation, add_generation_prompt=True)
            source_chat.append(_s_chat_id)
        src_len = [torch.LongTensor(_src_tokenized).ne(tokenizer.pad_token_id).sum().item() for _src_tokenized in source_chat]
        ...
        # input_ids = [torch.cat(torch.LongTensor(_src_tokenized), torch.LongTensor(_tgt_tokenized)) 
        #                 for _src_tokenized, _tgt_tokenized in zip(src_tokenized["input_ids"], tgt_tokenized["input_ids"])]
        input_ids = [torch.cat((torch.LongTensor(_src_tokenized), torch.LongTensor(_tgt_tokenized[1:]), torch.LongTensor([tokenizer.eos_token_id]))) 
                        for _src_tokenized, _tgt_tokenized in zip(source_chat, tgt_tokenized["input_ids"])]
    ...
    if torch.distributed.get_rank() == 0:
        ...
        print(tokenizer.decode(_sample["input_ids"], skip_special_tokens=False))
